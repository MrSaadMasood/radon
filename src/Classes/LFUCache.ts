...
export class LFUCache<T> extends EvictionPolicy<T> {
  ...
  public updateCache({
    ...
    if (alreadyCached) {
      ...
      minHeap.deleteFromMinHeapAndUpdateWithNewValue({
        key: cacheItem.key,
        TTL: -1,
      });
    ...
    if (!alreadyCached) {
      return currentMaxFreq > 1
        ? this.insertOnQueueTop(cacheItem as LFU)
        : this.enqueue(cacheItem);
    }
  }

  private insertBasedOnFrequency(
    deletedNode: NodeOrNull<T>,
    ...
    while (
      curr.next &&
      curr.next.val.type === "LFU" &&
      curr.next.val.frequency < updatedFrequency
    ) {
      curr = curr.next;
    }
  ...
  public adjustFrequency(freqToUpdate: number, operation: "INC" | "DEC") {
    const totalNodesWithThisFrequency =
      this.nodesWithGivenFrequency(freqToUpdate);
    switch (operation) {
      case "INC":
