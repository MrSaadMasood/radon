...
export function cleanUpExpiredKeys(
  ...
  for (let i = 0, len = cachedNodes.length; i < len; i++) {
    ...
    if (!expiredNodes[cachedNode.key])
      cachedNodeWithExpiredKeysRemoved.push(cachedNode);
  }

  return {
    storeAfterCleanup: store,
    useableHeap,
    cachedNodeWithExpiredKeysRemoved,
  };
}

export function cacheUseableNodes(
  nodes: NodeValue[],
  evictionPolicy: EvictionClasses,
) {
...
export function isKeyExpired(storedObject: MapValueObject) {
  ...
  if (ttlExpirationValidator(storedObject.TTL + storedObject.timestamp))
    return true;
  else return false;
}
