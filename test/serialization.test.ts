...
import { cacheUseableNodes, cleanUpExpiredKeys, parseJsonFile } from "../src/utils/utils";
...
describe("tests the serialzation of InMemoryStore, min heap and the LRU / LFU Cache", () => {
  ...
  const toSerialize = {
    store: Array.from(new Map(Object.entries({
      one: { value: "value1" },
  ...
  it("should clean up expired keys successfully", () => {
    ...
    expect(cachedNodeWithExpiredKeysRemoved.length).toEqual(storeAfterCleanup.size)
  ...
  it("should add the cleanedUp nodes in the cache", () => {
    ...
    const { store, heap, cachedNodes } = toSerialize
    const { cachedNodeWithExpiredKeysRemoved } =
      cleanUpExpiredKeys(new Map(store), heap, cachedNodes)
