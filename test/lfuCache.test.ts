...
describe("should test the functionality of LFU caching and eviction mechanism", () => {
  ...
  it("should single node insertion and frequency updation", () => {
    ...
    lfuCache.updateCache({
      cacheItem: { key: "one", type: "LFU", frequency: 1 },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
  ...
  it("should add the node with updated frequency in correct order", () => {
    ...
    lfuCache.updateCache({
      cacheItem: { key: "two", type: "LFU", frequency: 1 },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
    lfuCache.updateCache({
      cacheItem: { key: "three", type: "LFU", frequency: 1 },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
    lfuCache.updateCache({
      cacheItem: { key: "four", type: "LFU", frequency: 1 },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
  ...
  it("should check if the eviction policy is applied", () => {
    ...
    lfuCache.updateCache({
      cacheItem: { key: "five", type: "LFU", frequency: 1 },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
