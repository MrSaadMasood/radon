...
describe("should test the lru cache management", () => {
  ...
  it("should correctly update the lru cache", () => {
    ...
    lruCache.updateCache({
      cacheItem: { key: "one", type: "LRU", timestamp: Date.now() },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
    lruCache.updateCache({
      cacheItem: { key: "two", type: "LRU", timestamp: Date.now() },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
    lruCache.updateCache({
      cacheItem: { key: "three", type: "LRU", timestamp: Date.now() },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
    ...
    lruCache.updateCache({
      cacheItem: { key: "four", type: "LRU", timestamp: Date.now() },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
    ...
    lruCache.updateCache({
      cacheItem: { key: "four", type: "LRU", timestamp: 1 },
      storeCapacity,
      inMemoryStore,
      minHeap,
    });
